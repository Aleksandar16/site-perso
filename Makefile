...
db:
	...
	$(EXEC) bin/console doctrine:schema:update --force --complete
...
yarn-upgrade-latest:
	$(EXEC) yarn upgrade-interactive --latest
...
composer-update-interactive:
	$(EXEC) composer update --interactive
...
ifndef TEST
	...
	docker-compose run cypress npx cypress run --spec "cypress/e2e/$(TEST)"
endif
